...
class MetaCorgiSnacks
  ...
  #
  #   tastiness_method = 'get_' + method_name + "_tastiness"
  ...
  def self.define_snack(name)
    define_method(name) {
      method1 = "get_#{name}_info"
      method2 = "get_#{name}_tastiness"

      info = @snack_box.send(method1, @box_id)
      tastiness = @snack_box.send(method2, @box_id)
